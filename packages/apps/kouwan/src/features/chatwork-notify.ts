...
import { CHATWORK_ROOM_ID, CHATWORK_TOKEN } from "../config/environment";
...
kintone.events.on(["app.record.detail.show"], (event: any) => {
  const header = document.querySelector<HTMLElement>(".gaia-argoui-app-toolbar-statusmenu");
  ...
  btnSend.onclick = () => {
    const preview = (kintone.app.record.get() as any).record.preview.value;
    if (!preview || !preview.trim()) {
      void Swal.fire({
        title: "送信内容がありません",
        text: "先に「送信内容更新」で内容を作成してください。",
        icon: "warning",
      });
      return;
    }
...
function keyHandler(event: KeyboardEvent) {
  ...
  if (event.altKey && event.shiftKey && event.key === "Delete") {
    ...
    if (parseInt(msgId)) {
      ...
      // 先清 preview 再清 msgId，避免第二个请求触发的 reload 把第一个请求打断。
      void recordPost("preview", "", false).then(() => recordPost("msgId", "", true));
    }
  }
}
...
kintone.events.on(
  ...
  (event: any) => {
    ...
    event.record.table.value.forEach((row: any) => (row.value.deadline.disabled = true));
    return event;
  },
);
...
kintone.events.on(
  ...
  (event: any) => {
    ...
    row.shipper.value = COMPANY_CODE_MAP[row.shipper.value] || row.shipper.value;
